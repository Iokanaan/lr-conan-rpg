...
import { processQualitiesTag } from "./qualities"

export const handleAttack = function(sheet: Sheet<CharData>, component: Component) {
    ...
    if(entryData.type_Choice === "melee") {
        ...
        if(!volee && !charges) {
            sheet.prompt('Erreur', 'GenericPrompt',function(){}, function(promptSheet: Sheet<YesNoData>) {
...
export const handleThrow = function(sheet: Sheet<CharData>, component: Component) {
    const entryData = sheet.get("weapons").value()[component.index()]
    entryData.damage_Input = (entryData.damage_Input !== undefined) ? entryData.damage_Input : 0 
    const tags = processQualitiesTag(entryData)
    tags.push("attack")
